...
请一次性生成这个知识点的全部教学内容，包含三部分：

1. intro（核心概念）：用3-5句话介绍定义和核心思想。如果有关键公式或定义直接给出。不要举例题。
2. detail（详细讲解）：讲解计算方法或解题步骤、关键技巧和注意事项、常见错误及避免方法。不要重复例题。
3. examples（例题）：提供3道例题（简单、中等、较难各一道），每道含题目、分步解答、解析说明。
